...
export function PageSection({ children, columns = 'default' }) {
  const className = columns === 'sidebar' ? 'ui-page-section ui-page-section--sidebar' : columns === 'tight' ? 'ui-page-section ui-page-section--tight' : 'ui-page-section';
...
export function StatusBanner({ tone = 'neutral', items = [], detail, actions }) {
  return (
    <div className={`ui-status-banner ui-status-banner--${tone}`}>
...
export function StateBlock({ type = 'empty', title, detail, action }) {
  return (
    <div className={`ui-state ui-state--${type}`}>
